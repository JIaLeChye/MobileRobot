...
class Encoder:
    ...
    def setup(self):
        ...
        GPIO.setup(self.LEFT_HALLSEN, GPIO.IN, pull_up_down=GPIO.PUD_UP)
        GPIO.setup(self.RIGHT_HALLSEN, GPIO.IN, pull_up_down=GPIO.PUD_UP)
        ...
        while retry_count > 0:
            try:
                ...
                GPIO.setup(self.LEFT_HALLSEN, GPIO.IN, pull_up_down=GPIO.PUD_UP)
                GPIO.setup(self.RIGHT_HALLSEN, GPIO.IN, pull_up_down=GPIO.PUD_UP)
                GPIO.add_event_detect(self.LEFT_HALLSEN, GPIO.BOTH, callback=self.left_update, bouncetime=500)
                GPIO.add_event_detect(self.RIGHT_HALLSEN, GPIO.BOTH, callback=self.right_update, bouncetime=500)
                if self.debug:
